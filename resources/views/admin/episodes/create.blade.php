...
<div class="max-w-4xl mx-auto">
    <div class="bg-gray-800 rounded-2xl shadow-2xl p-8">
        ...
        <form action="{{ route('admin.seasons.episodes.store', [$movie, $season]) }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-2">Số tập</label>
                    <input type="number" name="episode_number" value="{{ old('episode_number', $season->episodes->max('episode_number') + 1 ?? 1) }}"
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $movie->type == 1 ? 'Full' : '') }}"
